...
# Relaton::Gost::Item, re-serialize, and diff against the source.
# Exit 1 on any mismatch or missing primary docidentifier.
#
# Uses Relaton::Gost::Item so the GOST-specific ext fields (urn,
# webpage) round-trip natively — no merge hack.
...
Dir[path].sort.each do |f|
  ...
  item = Relaton::Gost::Item.from_yaml(yaml)
